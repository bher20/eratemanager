    ...
    parser_src = textwrap.dedent(f"""\
        ...
        func init() {{
        	RegisterParser(ParserConfig{{
        		Key:       "{key}",
        		Name:      "{name}",
        		ParsePDF:  Parse{key_cap}RatesFromPDF,
        		ParseText: Parse{key_cap}RatesFromText,
        	}})
        }}

        // Parse{key_cap}RatesFromPDF opens a {name} rates PDF at the given path,
        // extracts text, and delegates to Parse{key_cap}RatesFromText.
        func Parse{key_cap}RatesFromPDF(path string) (*RatesResponse, error) {{
        	...
        	return Parse{key_cap}RatesFromText(buf.String())
        }}

        // Parse{key_cap}RatesFromText parses a plain-text representation of the
        ...
        func Parse{key_cap}RatesFromText(text string) (*RatesResponse, error) {{
...
`
    ...
    print("=" * 60)
    print("Scaffolded provider files:")
    ...
    print()
    print("=" * 60)
    print("NEXT STEPS (only 2 required!):")
    print("=" * 60)
    print()
    ...
    print(f"   }},")
    print()
    print("2) Customize the regex patterns in the generated parser file:")
    print(f"   internal/rates/parser_{key}_pdf.go")
    print()
    print("That's it! The parser auto-registers via init().")
    print()
    print("=" * 60)
    print("OPTIONAL: Additional integrations")
    print("=" * 60)
    print()
    print("3) Add a provider entry to the Home Assistant integration's const.py:")
    print()
    print(f"   \"{key}\": {{")
    print(f"       \"name\": \"{name}\",")
    ...
    print(f"   }},")
    print()
    print("4) Add a pdf URL entry in Helm values.yaml:")
    print()
    print(f"   pdf:")
    print(f"     {key}:")
    print(f"       url: \"https://example.com/path/to/rates.pdf\"")
    ...
    print("=" * 60)
    print()
    print(f"   gofmt -w internal/rates/parser_{key}_pdf.go internal/rates/parser_{key}_pdf_test.go")
    print("   go test ./...")
    print()
